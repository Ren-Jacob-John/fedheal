...
| `DenseNet201ChestXrayModel` | ✅ Addressed | `BiomedCLIPChestXrayModel` added as the current-tier default (BiomedCLIP embedding + a fine-tuned CNN+ViT-hybrid head — see `models/foundation_biomedclip.py`); `DenseNet201ChestXrayModel` kept registered as the auto-degrade fallback for low-resource/no-HF-access hospitals, per `registry.py` | Done — real code, untested in this sandbox (no HF network access), same honest-and-marked pattern as this project's other foundation-model additions |
| `ResNet50RetinaModel` | ✅ Addressed | `BiomedCLIPRetinaModel` added as the current-tier default (see `models/foundation_biomedclip.py`); `ResNet50RetinaModel` kept as fallback | Done — same caveat as above |
| `EfficientNetSkinLesionModel` | ✅ Addressed | `BiomedCLIPSkinModel` added as the current-tier default (see `models/foundation_biomedclip.py`); `EfficientNetSkinLesionModel` kept as fallback | Done — same caveat as above |
| `UNetSegmentationModel` | ✅ Addressed | `NNUNetSegmentationModel` added as the current-tier default for `ct_scan` when a hospital has a trained model folder (see `models/foundation_nnunet.py`); `UNetSegmentationModel` kept as the no-training-required fallback. `BiomedParse`/`SegVol` (already registered under their own `prompted_segmentation`/`ct_volumetric` modalities from an earlier pass) remain the right current-tier pick for prompted segmentation without much per-task labeled data | Done — real code, untested here (nnU-Net needs a real trained model folder + GPU; BiomedParse/SegVol still blocked on GPU + HF access as before) |
...
| Histopathology track (`module6-condition-router`'s `HistopathologyMILModel`) | ✅ Addressed | `FoundationPathologyMILModel` added as the current-tier default — real UNI2-h patch embeddings feeding an actual CLAM-style *gated*-attention MIL head (see `module6-condition-router/models/foundation_pathology_mil.py`); legacy `HistopathologyMILModel` kept registered as the fallback | Done — real code, untested here (`timm`, HF Hub authorization for gated weights, and GPU all unavailable in this sandbox) |
| *(was: still missing entirely)* | ✅ Addressed | Genomic variant track added: `Evo2VariantModel`, a new `genomic_variant` specialist in `module5-modelzoo/registry.py` (see `models/foundation_evo2.py`), wired into `module6-condition-router`'s `breast_cancer`/`leukemia` conditions as `genomic_variant_breast_cancer`/`genomic_variant_leukemia` — directly covers the named BRCA1/2 and leukemia-cytogenetics use cases. The existing sklearn-family `GenomicExpressionModel` (Random Forest) is kept registered too, as the documented fallback for small cohorts / expression-panel-only data — the two take different input contracts (variant calls vs. expression panel), so they're additive, not a strict swap; see `foundation_evo2.py`'s module docstring | Done — real code, untested here (no GPU/HF access) |
| *(new, found during this pass)* Inception v3/v4, VGG16/19, Capsule Networks, Mask R-CNN, DeepLab v3/v3+, early fusion, TabNet, plain MLP-on-tabular, SVM-on-tabular | ✅ Audited — never present | Nothing to swap; these were only ever named in this catalog's own reference menu (Sections 1–4, 9) as *what not to use*, not implemented anywhere in `module5-modelzoo`/`module6-condition-router`/the rest of the repo | Done — confirmed via repo-wide search, no removal needed |
